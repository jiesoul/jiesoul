...
(re-frame/reg-event-db
 ::f-state/set-modal-show?
 (fn [db [_ state]]
   (assoc-in db [:modal :show?] state)))
...
(defn modal-component [props & children]
  (let []
    (r/create-class
     {:display-name "modal-component"
      :component-did-mount (fn [this]
                             (println "modal component-did-mount"))
      :component-did-update (fn [this old-argv]
                              (let [new-argv (rest (r/argv this))]
                                ()))
      
      :reagent-render (fn [{:keys [id show? title on-close]} & children]
                        [:div {:id id
                               :tab-index "-1"
                               :class (str (if show? "" "hidden ") "flex overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-40 
                                        justify-center items-center w-full inset-0 h-[calc(100%-1rem)] max-h-full")
